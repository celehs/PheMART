...
def parse_arguments(parser):
    ...
    parser.add_argument('--file_snps_labeled_embedding', type=str,
                        default="Binary_labeled_230201_clinvar_embedding.npy",
    ...
    parser.add_argument('--filename_eval', type=str, default="results_validations.txt",
...
for rowi in range(len(CUIs)):
    dic_cui_emb[str(CUIs[rowi])] = np.concatenate((np.array(embedding_LLM[rowi]), np.array(embedding_EHR[rowi])),
                                                  axis=-1)
...
def Model_PheMART(input_dim1=768, input_dim2=768 + 300, kl_weight=weight_kl, latent_dim=64, tau_KL=0.1):
    ...
    encoder1_layer1_residual = layers.Dense(120, activation=tf.nn.leaky_relu, name="encoder1/fcn1_residual",
                                            dtype='float32')
    encoder1_layer1_residual_h1 = layers.Dense(120, activation=tf.nn.leaky_relu, name="encoder1/fcn1_residual_h1",
                                               dtype='float32')
    encoder1_layer2_mean = layers.Dense(int(1.5 * latent_dim), activation=tf.nn.leaky_relu, name="encoder1/fcn2/mean",
                                        dtype='float32')
    ...
    encoder2_layer1_1 = layers.Dense(int(1.5 * latent_dim), activation=tf.nn.relu, name="encoder2/fcn1",
                                     dtype='float32')
    encoder2_layer1_2 = layers.Dense(int(48), activation=tf.nn.relu, name="encoder2/fcn1_2", dtype='float32')
    encoder2_layer2_mean = layers.Dense(int(1.2 * latent_dim), activation=tf.nn.leaky_relu, name="encoder2/fcn2/mean",
                                        dtype='float32')

    encoder2_layer3 = layers.Dense(latent_dim, activation=tf.nn.leaky_relu, name="encoder2/fcn3", dtype='float32')
    decoder1_layer1 = layers.Dense(int(input_dim1 * 1.5), activation=tf.nn.leaky_relu, name="decoder1/fcn1",
                                   dtype='float32')
    ...
    decoder2_layer1_2 = layers.Dense(int(300 * 1.5), activation=tf.nn.leaky_relu, name="decoder2/fcn1_2",
                                     dtype='float32')
    ...
    output = (tf.reduce_sum(tf.multiply(feature1_l, feature2_l), axis=-1)) / (
            tf.sqrt(tf.reduce_sum(tf.square(feature1_l), axis=-1)) * tf.sqrt(
        tf.reduce_sum(tf.square(feature2_l), axis=-1)))

    feature1_u, loss_vae1_u, mean1_ppi_n = AE1(input1_u, input1_u_gene)
    feature2_u, loss_vae2_u, mean2_u = AE2(input2_u)
    loss_vae = (loss_vae1_l + loss_vae2_l) / 2.0 + weight_unlabel_snps * (
                loss_vae1_u + loss_vae2_u) / 2.0  # + loss_distill*weight_distill

    model = models.Model(inputs=[input1_l, input2_l, input1_u, input1_u_gene, input2_u, input1_l_gene, input_gene_ppi_1,
                                 input_gene_ppi_2,
                                 input1_l_p, input1_l_p_gene, input1_l_n, input1_l_n_gene],
                         outputs=[output, loss_vae, feature1_l, feature2_l, feature1_ppi_p, feature1_u, feature1_l_raw,
                                  feature1_l_p, feature1_l_n])
    return model
...
def train_step(model, epoch_num, input_pro_tsr, input_dis_tsr, label, unlabel_snps, unlabel_snps_gene,
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        prediction, loss_vae, feature_snps, feature_cuis, feature1_l_gene_ppi_p, feature1_l_gene_ppi_n, feature_snp_mean, feature_snp_mean_p, feature_snp_mean_n = \
            model([input_pro_tsr, input_dis_tsr, unlabel_snps, unlabel_snps_gene, unlabel_disease, input_gene_emb,
                   input_gene_ppi1_emb, input_gene_ppi2_emb,
                   input_pro_tsr_positive, input_pro_tsr_positive_gene, input_pro_tsr_negative,
                   input_pro_tsr_negative_gene])
        ...
        similarity_PPI_p = (tf.reduce_sum(tf.multiply(feature_snp_mean, feature1_l_gene_ppi_p), axis=-1)) / (
                    tf.sqrt(tf.reduce_sum(tf.square(feature_snp_mean), axis=-1)) * tf.sqrt(
                tf.reduce_sum(tf.square(feature1_l_gene_ppi_p), axis=-1)))

        similarity_PPI_n = (tf.reduce_sum(tf.multiply(feature_snp_mean, feature1_l_gene_ppi_n), axis=-1)) / (
                tf.sqrt(tf.reduce_sum(tf.square(feature_snp_mean), axis=-1)) * tf.sqrt(
            tf.reduce_sum(tf.square(feature1_l_gene_ppi_n), axis=-1)))
        ...
        if tau_softmax > 0.5:
            loss_snps = cce(labels_multi,
                            tf.nn.softmax(tf.transpose(feature_interaction / tau_softmax), axis=-1)) * train_gene_weight
            loss_cui = cce(labels_multi, tf.nn.softmax(feature_interaction / tau_softmax, axis=-1)) * train_gene_weight
        else:
            loss_snps = cce(labels_multi,
                            tf.nn.softmax(tf.transpose(feature_interaction * 2.0 * tau_softmax_adjust / tau_softmax),
                                          axis=-1)) * train_gene_weight
            loss_cui = cce(labels_multi, tf.nn.softmax(feature_interaction * tau_softmax_adjust / tau_softmax,
                                                       axis=-1)) * train_gene_weight
        ...
        if weight_cosine >= 5:
            loss = tf.reduce_mean(train_gene_weight * (
                        (batch_size - tf.reduce_sum(label)) * label * distance_same * 0.99 + (
                            1 - label) * distance_differ * tf.reduce_sum(label)) / batch_size)
        else:
        ...
        if epoch_num < epochs / 3:
            if epoch_num < 6:
                ...
                loss_joint = loss_snp_contrast + weight_vae * loss_vae + loss_ppi * weight_ppi[
                    1] + loss_CLIP * 0.1 + loss * 5.0
        else:
            loss_joint = weight_vae * loss_vae + loss_CLIP * weight_CLIP + loss_ppi * weight_ppi[
                2] + loss_snp_contrast + loss * 5.0
...
def valid_step(model, input_pro_tsr, input_dis_tsr, label, input_gene_emb):
    prediction, loss_vae, feature_snps, feature_cuis, feature_ppi_1, feature_ppi_2, feature_snp_mean, feature_snp_mean_p, feature_snp_mean_n = \
        model([input_pro_tsr, input_dis_tsr, input_pro_tsr, input_gene_emb, input_dis_tsr,
               input_gene_emb, input_pro_tsr, input_gene_emb, input_pro_tsr, input_pro_tsr, input_pro_tsr,
               input_pro_tsr])
...
def train_model(model, ds_train, ds_test, eval_SNPs, eval_index, epochs, eval_SNPs_train, eval_cui_train,
                eval_gene_train, eval_gene_test):
    ...
    while (epoch_num < epochs):
        ...
        for traindata_snps, traindata_cuis, traindata_Y, unlabel_snps, unlabel_snps_gene, unlabel_disease, traindata_gene, traindata_gene_ppi_1, traindata_gene_ppi_2, train_gene_weight, \
            ...
            if True:
                loss, prediction, label = \
                    train_step(model, epoch_num, traindata_snps, traindata_cuis, traindata_Y, unlabel_snps,
                               unlabel_snps_gene,
                               unlabel_disease, traindata_gene, traindata_gene_ppi_1, traindata_gene_ppi_2,
                               train_gene_weight,
                               traindata_snps_positive, traindata_snps_positive_gene, traindata_snps_negative,
                               traindata_snps_negative_gene, train_gene_PPI_p1_weight)
        ...
        if epoch_num % epoch_show == 0 or epoch_num > epochs - 3:
            ...
            print("PRC_overall: ", PRC_overall, " prevelance: ", Prevelance_overall, "prevelance gain: ",
                  PRC_overall_gain)
            print(
                "---epoch: %i,  loss: %3f, VAE_loss: %3f, CLIP_loss: %3f, train_loss_ppi: %3f, train_AUC: %3f, auc_overall: %3f,AUC_SNPs: %3f, "
            ...
            if save_flag == False and epoch_num > epochs - 3:
                ...
                f.write(
                    "Time_pre: %2f min, weight_vae: %2f, AUC_train :%4f, AUC_overall:%4f, AUC_snps:%4f, AUC_disease:%4f,"
                    " prc_overall :%4f, prc_overall_gain :%4f, PRC_snps :%4f, PRC_disease:%4f,"
                    " MRR:%4f, recall10:%4f, recall50:%4f , rank_mean:%2f, rank_median:%2f " %
                    (time_preprocessing, weight_vae, AUC_overall_train, np.mean(AUC_overall), np.mean(AUC_SNPs),
                     np.mean(AUC_cuis),
                     PRC_overall, PRC_overall_gain, np.mean(PRC_SNPs), np.mean(PRC_cuis), MRR, recall10, recall50,
                     np.mean(rank_total), np.median(rank_total)))
                f.write("\r")
                f.close()
                print(
                    "------------------------------------------------- saving predictions end-----------------------------------")
                ...
                if True:
                    ...
                    for snps in snps_all_prediction:
                        if snps == snps and snps in dic_snpsname_emb_all:
                            ...
                            prediction_all, CUIs_all_covered = zip(
                                *sorted(zip(prediction_all, CUIs_all_covered), reverse=True))
                            df = pd.DataFrame({})
...
if __name__ == '__main__':
    ...
    (traindata_snps, traindata_cuis, traindata_snps_P, traindata_cuis_P, traindata_gene_P, traindata_Y, traindata_names,
     ...
     unlabel_snps, unlabel_gene, unlabel_disease, eval_SNPs_train, eval_cui_train, train_gene, train_gene_ppi_1,
     train_gene_ppi_2, test_gene, eval_gene_train, eval_gene_test, train_gene_weight,
    ...
    ds_train = tf.data.Dataset.from_tensor_slices(
        (traindata_snps, traindata_cuis, traindata_Y, unlabel_snps, unlabel_gene,
    ...
    train_model(model, ds_train, ds_test, eval_SNPs, eval_index, epochs, eval_SNPs_train, eval_cui_train,
                eval_gene_train, eval_gene_test)
